...
CATEGORY_ORDER = [
    ...
    "Development", "Games", "System", "Settings",
...
CATEGORY_RULES = [
    ...
    ("Development", ["Development", "IDE", "Debugger", "RevisionControl", "WebDevelopment"]),
    ("Games",       ["Game", "Emulator", "ArcadeGame", "BoardGame", "CardGame"]),
    ("System",      ["System", "TerminalEmulator", "FileManager", "Monitor", "PackageManager"]),
...
def _read_events(fd):
    """Baca semua event yang tersedia dari inotify fd, return list nama file."""
    raw = os.read(fd, 4096)
    names = []
    offset = 0
    while offset < len(raw):
        ...
        if length:
            name = raw[offset: offset + length].rstrip(b"\x00").decode(errors="replace")
...
ICON_SEARCH_DIRS = [
    Path.home() / ".local/share/icons/Tela-light/scalable/apps",
    Path.home() / ".local/share/icons/Tela/scalable/apps",
...
def parse_desktop_file(path: Path) -> dict | None:
    """
    Parse satu file .desktop, return dict atau None kalau harus di-skip.
    Menggunakan configparser stdlib — tidak ada subprocess/awk/sed.
    """
    try:
    ...
    # configparser butuh section header — .desktop sudah punya [Desktop Entry]
    ...
    # Skip kondisi yang tidak perlu ditampilkan
    ...
    # Bersihkan placeholder %f %F %u %U %d %D %n %N %i %c %k %v %m
    import re
...
def build_menu() -> dict:
    """
    Scan semua direktori .desktop, bangun struktur menu.
    Deduplicate berdasarkan (name, exec).
    """
    seen: set[tuple] = set()
    ...
    # Sort apps per kategori secara alfabetis
    for cat in categories:
        categories[cat].sort(key=lambda a: a["name"].lower())

    # Susun output — hanya kategori yang tidak kosong
    result = {
        "categories": [
            {
                "category": cat,
...
def run_daemon():
    ...
    # Tambahkan watch untuk setiap direktori yang ada
    ...
    # Generate pertama kali saat startup
    generate_once()
    ...
    while True:
        # Hitung timeout: kalau ada event pending, tunggu sampai debounce selesai
        if pending_regen:
            timeout = max(0.0, deadline - time.monotonic())
        else:
            timeout = None  # block selamanya sampai ada event

        # select() — tidur sampai ada event inotify atau debounce timeout
        readable, _, _ = select.select([ifd], [], [], timeout)

        if readable:
            # Ada event masuk — baca dan set/reset debounce timer
            changed_files = _read_events(ifd)
            if changed_files:
                pending_regen = True
                deadline = time.monotonic() + DEBOUNCE_SECS
                print(
                    f"[menu-gen] Event terdeteksi: {changed_files} "
                    f"— debounce {DEBOUNCE_SECS}s...",
                    flush=True,
                )
        else:
            # Timeout habis = debounce selesai, waktunya regenerate
            if pending_regen:
                generate_once()
                pending_regen = False
